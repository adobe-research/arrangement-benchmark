    ...
    for (const auto& mat : matrices) {
        ...
        if (mat.cols() != num_cols) {
            ...
            err_msg << "Cannot vstack because "
                    << "matrix have different number of columns" << std::endl
                    << "Expect " << num_cols << " but get " << mat.cols() << "columns";
...
{
    typedef Eigen::Matrix<typename T::Scalar, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>
        OutType;
    if (rows.empty()) {
    ...
    for (const auto& row : rows) {
        if (row.size() != num_cols) {
            std::stringstream err_msg;
            err_msg << "Expect vectors are of size " << num_cols << " but get " << row.size()
                    << " at row " << row_count;
            throw RuntimeError(err_msg.str());
        }

        std::copy(row.data(), row.data() + num_cols, result.data() + row_count * num_cols);
        row_count++;
    }
...
{
    typedef Eigen::Matrix<typename T::Scalar, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor>
        OutType;
    if (cols.empty()) {
    ...
    for (const auto& col : cols) {
        if (col.size() != num_rows) {
            std::stringstream err_msg;
            err_msg << "Expect vectors are of size " << num_rows << " but get " << col.size()
                    << " at row " << col_count;
            throw RuntimeError(err_msg.str());
        }

        std::copy(col.data(), col.data() + num_rows, result.data() + col_count * num_rows);
        col_count++;
    }
...
template <typename T>
Eigen::Matrix<T, Eigen::Dynamic, 1> MatrixUtils::std2eigen(const std::vector<T>& data)
{
...
Eigen::Map<T> MatrixUtils::reshape(Eigen::MatrixBase<Derived>& M, size_t rows, size_t cols)
{
    ...
    if (num_entries != rows * cols) {
        std::stringstream err_msg;
        err_msg << "Cannot reshape matrix of size (" << ori_num_rows << ", " << ori_num_cols
                << ") into matrix of size (" << rows << ", " << cols << ")" << std::endl;
        throw RuntimeError(err_msg.str());
    }
...
{
    return Eigen::Map<const T>(
        static_cast<const Derived&>(matrix).data(), matrix.rows() * matrix.cols());
}
